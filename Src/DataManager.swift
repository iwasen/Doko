...
class DataManager {
    var lang: Int = 0
    var readContinueFlag: Bool = false
    var readCurrentPage: Int = 0
    var playContinueFlag: Bool = false
    var playCurrentPage: Int = 0

    var dataExistFlag: Bool = false
    var playFindItem = [[PlayFindItem]](repeating: [PlayFindItem](repeating: PlayFindItem(), count: FIND_ITEM_NUM), count: PAGE_NUM)
    ...
    func initPlayFindPage(page: Int)
    {
        var r = 0
    ...
    func getBgMovieFile(page: Int) -> String
    {
        return String(format: "kddk1a_loop_p%02d", page + 1)
    }

    func getBgImageFile(page: Int) -> String
    {
        return String(format: "kddk1a_loop_p%02d_stl", page + 1)
    }

    func getBgSoundFile(page: Int) -> String
    {
        return String(format: "kddk1a_loop_p%02d", page + 1)
    }

    func getCompleteMovieFile(page: Int) -> String
    {
        let page = 8
        return String(format: "doko_%02dbg_complete", page + 1)
    }

    func getPlayItemName(page: Int, index: Int) -> String
    {
        return playPageData[page].itemData[index].itemName[lang]
    }

    func getPlayAtariRect(page: Int, index: Int) -> CGRect
    {
        return playPageData[page].itemData[index].atariRect
    }

    func getPlaySoundFile(page: Int, index: Int) -> String
    {
        return playPageData[page].itemData[index].soundFile[lang]
    }

    func getReadSentenceSoundFile(page: Int, viewIndex: Int, lineIndex: Int, sentenceIndex: Int) -> String
    {
        return String(format: "kddk1%@_re%02d_%d%@%d", lang == LANG_JPN ? "j" : "e", page + 1, viewIndex + 1, lineIndex == 0 ? "a" : "b", sentenceIndex + 1)
    }

    func getReadTextFile(page: Int) -> String
    {
        return String(format: "kddk1%@_re%02d", lang == LANG_JPN ? "j" : "e", page + 1)
    }

    func getReadZoomPoint(page: Int) -> CGPoint
    {
        return readZoomData[page].point
    }

    func getReadZoomRatio(page: Int) -> CGFloat
    {
        return readZoomData[page].ratio
    }

    func getImage(jpnFileName: String, engFileName: String) -> UIImage
    {
